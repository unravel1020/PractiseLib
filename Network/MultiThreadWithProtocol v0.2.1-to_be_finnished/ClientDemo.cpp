...
SOCKET reconnect(int max_retries = 5, int retry_interval_seconds = 3) {
    for (int i = 0; i < max_retries && is_running; ++i) {
        LOG(LogLevel::INFO) << "Attempting to reconnect (" << i + 1 << "/" << max_retries << ")...";
        SOCKET sock = connect_to_server();
        if (sock != INVALID_SOCKET) {
            LOG(LogLevel::INFO) << "Reconnected successfully.";
            return sock;
        }
        std::this_thread::sleep_for(std::chrono::seconds(retry_interval_seconds));
    }

    LOG(LogLevel::ERR) << "Failed to reconnect after " << max_retries << " attempts.";
    return INVALID_SOCKET;
}
...
int main() {
    ...
    while (is_running) {
        if (sock == INVALID_SOCKET) {
            ...
            heartbeat_thread = std::thread(send_heartbeat, sock);
            heartbeat_thread.detach();
        }
